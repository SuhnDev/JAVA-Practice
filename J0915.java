...
public class J0915 {
    public static void main(String[] args) {
        ...
        while(true) {
            ...
            try { //try catch 예외처리
                if(input.charAt(0)=='q') {  //q를 입력받으면 무한반복 탈출
                    break;
                }
                else { //숫자 입력
                    score = Integer.parseInt(input); //문자열을 정수형으로 변환
                    sum+=score; //sum에 score값을 추가
                    many++; //카운트 증가
                }
            } catch(NumberFormatException e) { 
                //문자열을 숫자로 바꿀 수 없을 때 예외처리
                System.out.print(ex); //ex 예외 출력
            }
        }
        
        if(many==0) {
            System.out.print(ex); //카운트 값이 0인데 무한루프 탈출시 예외 출력
        }
        else {
            ...
            System.out.print(many+"명의 평균 성적은 "+avg+" "+gra+" 입니다."); //결과 출력
        }
    }
}